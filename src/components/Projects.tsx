...
export default function Projects({ viewMode = 'full', onNavigateToProjectsPage, onBackToHome }: ProjectsProps) {
  ...
  if (viewMode === 'carousel') {
    return (
      <section id="projects" className="relative py-16 bg-white dark:bg-slate-950 overflow-hidden border-b border-slate-200/60 dark:border-slate-800/40 transition-colors duration-300">
              ...
              {Array.from({ length: maxIndex + 1 }).map((_, idx) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-200 cursor-pointer ${
                    activeIndex === idx ? 'bg-emerald-600 w-6' : 'bg-slate-200 hover:bg-slate-300 w-1.5'
                  }`}
    ...
    <section id="projects" className="relative py-16 bg-white dark:bg-slate-950 overflow-hidden border-b border-slate-200/60 dark:border-slate-800/40 transition-colors duration-300">
      ...
      {selectedProject && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div
            id="projects-modal-layout"
            className="w-full max-w-3xl bg-white rounded-3xl border border-slate-200 p-6 md:p-8 relative max-h-[95vh] overflow-y-auto shadow-2xl shadow-slate-900/10 animate-scaleUp"
          >
            <button
              onClick={() => setSelectedProject(null)}
              className="absolute top-5 right-5 p-2 rounded-full bg-slate-100 text-slate-500 hover:text-slate-800 hover:bg-slate-200 transition-all cursor-pointer"
              aria-label="Close project modal dialog"
            >
              <Minimize2 className="w-5 h-5" />
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start mb-8">
              ...
              <div className={`${selectedProject.description ? 'md:col-span-7' : 'md:col-span-12'} rounded-2xl overflow-hidden border border-slate-200 aspect-3/2`}>
                <ImageSlider
                  images={selectedProject.images}
                  alt={selectedProject.title}
                />
